...
\begin{algorithm}[H]
    ...
    \begin{algorithmic}[1]
        ...
        \For{$r$ iterations}
            \ForAll{capsule $i$ in layer $l$} $c_{i}\gets softmax(b_i)$ \Comment{See equation \ref{eq:softmax}}\EndFor
            \ForAll{capsule $j$ in layer $(l + 1)$} $s_{j}\gets \sum_ic_{ij}\hat{u}_{j|i}$ \EndFor
            \ForAll{capsule $j$ in layer $(l + 1)$} $v_{j}\gets squash(s_j)$ \Comment{See equation \ref{eq:squash}}\EndFor
